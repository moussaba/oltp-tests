...
class JobRunBenchmark < SwrJob
  ...
  def get_jenkins_parameters
    @drive = ENV['DRIVE_TO_BASELINE']
    @fio_config_file = ENV['FIO_CONFIGFILE']
  end
  ...
  def doit(verbose)
    ...
    if @fio_config_file != nil
      solution_config = "config/#{@fio_config_file}"
    else
      solution_config = ""
    end 
    ...
    vm_threads = Hash.new
    @config["vms"].each do |vm,prop,drive,config_file|
      puts "Creating thread for #{vm}"
      cmd = "cd ~/workspace/#{@job_name};sudo java -jar bin/jruby-test.jar jobs/baseline_fio.rb #{@config_file} #{@drive} "
