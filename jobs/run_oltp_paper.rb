...
class JobRunBenchmark < SwrJob
  ...
  def setup(verbose)
    ...
    case ENV["SWR_TEST"]
      when "sysbench"
        @config["benchmark"]["named_args"]["num-threads"] = ENV["SWR_BENCHMARK_THREADS"]
      when "sysbench-readonly"
        @config["benchmark"]["named_args"]["num-threads"] = ENV["SWR_BENCHMARK_THREADS"]
      when "tpcc"
        @config["benchmark"]["short_args"]["c"] = ENV["SWR_BENCHMARK_THREADS"]
    end
    ...
    if copy_data? || pre_condition?
      ...
      shell.execute("sudo mkfs.ext4 -E nodiscard -F #{drive}",true)
      puts "------mkdirs-------------"
     ...
     puts "------mount--------------"
     @fileutils.su_mount drive + " -o nobarrier " + config["mysql"]["named_args"]["datadir"]
     #cmd = "sudo rm -rf " + @config["mysql"]["named_args"]["datadir"] + "/ib_logf*"
     #shell.execute(cmd,true)
    end
    ...
    if diskstats?
        drive = ENV["SWR_DRIVE"]
        @config["benchmark"]["stats_binary"]["named_args"]["devices-regex"] = drive.split("/").last
        @config["benchmark"]["stats_binary"]["named_args"]["iterations"] = ENV["SWR_RUNTIME"].to_i + 5
    end
  ...
  def start_mysql(buffer_pool_size,verbose)
    bpool = ENV["SWR_BUFFER_POOL"]
    logsize = ENV["SWR_LOGSIZE"]
    logbuffersize = ENV["SWR_LOG_BUFFER_SIZE"]
    puts "------starting mysqld with buffer_pool_size=#{bpool}------------"
    @mysqld = SwrMysqld.new(@config["mysql"])
    @mysqld.args["named_args"]["innodb_buffer_pool_size"] = bpool
    @mysqld.args["named_args"]["innodb_log_file_size"] = logsize
    @mysqld.args["named_args"]["innodb_log_buffer_size"] = logbuffersize
    puts "LOGSIZE WAS #{logsize}"
    @mysqld.args["named_args"]["innodb_read_io_threads"] = ENV["SWR_IO_THREADS"]
    @mysqld.args["named_args"]["innodb_write_io_threads"] = ENV["SWR_IO_THREADS"]
    @mysqld.start(verbose=true)
    sleep 60
...
if diskstats != nil
    if diskstats == "false"
        puts "DISKSTATS NOT SELECTED"
        configs << "config/scenario/24G_1trial_1800.yaml"
    else    
        puts "DISKSTATS SELECTED"
        configs << "config/scenario/24G_1trial_1800_diskstats.yaml"
    end
end
